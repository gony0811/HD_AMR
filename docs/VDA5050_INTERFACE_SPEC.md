...
| `params.anchorGroupId` | 정렬(anchor) 공유 그룹 — **같은 그룹의 연속 액션은 사이에 주행이 없었다면 정렬 재수행 생략 가능**. AMR 구현: 그룹 첫 액션이 ⑤~⑯에서 작업물 좌표계(T_N)를 등록하면, 이후 액션은 **정렬 스텝군(②③④+⑤~⑯)을 건너뛰고** ⑱ 검사 수행부터 실행한다(⑱이 user:N 기준으로 원점 이동부터 하므로 재정렬이 불필요). 무효화: 주행 발생·시퀀스 실패·그룹 변경·신규 order. 코봇 홈 복귀는 **노드의 마지막 검사 액션에서만** 수행한다(액션 사이 홈 왕복 금지) |
