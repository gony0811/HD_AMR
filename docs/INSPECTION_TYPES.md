...
5. **profileId enum 구현** — `ref.area_task.profile_id` / 액션 `param_schema` 반영.
   ...
   > 배선 완료(VDA5050_INTERFACE_SPEC §8.5.1 (5)). **`CROSS3-*`·`CORNER2` 는 미시드(신규 — §9-6 후속).**
...
6. **신규 타입(CROSS3·CORNER2) 구현** `[진행 중]` — 카탈로그 정본화(2026-09-15, 독립 5종) 완료. 후속:
   - **CROSS3(3갈래)**: CROSS4와 동형 — **캡처 교시로 단일화**. X-Y 6-DOF 캡처 교시(`/inspection-points`)로 경유점 확보, `CROSS3-*` 5종 레시피 시드. (평면 3-arm 패턴 생성은 교시 시작 템플릿 용도로만 잔존 — 런타임 수식 실행 아님)
