...
| **T3 (T자)** | **3** | 용접선이 T자로 만나는 3갈래 교차 | 면 내부 격자 교차 |
...
→ **형상 = 5종** (LINE·T3·CROSS4·CORNER2·CORNER3). 접미사 숫자 = **팔 갈래 수(T3/CROSS4)** 또는 **접합 면 수(CORNER2/CORNER3)**.

> **교시/실행 방식 요약** — 상세 §5:
> - **LINE**: 도면(X-Z 단면) 솎기 (현행).
> - **T3·CROSS4**: **이원화** — 평탄면이면 수식 생성(패턴), 코로게이션 등 굴곡면이면 X-Y 6-DOF 캡처 교시(`/inspection-points`).
...
**면 위 junction(LINE·T3·CROSS4)** 은 면 자세 5군과 결합한다. **코너(CORNER2·CORNER3)** 는 면 자세 무관(코너 자체가 면 배치를 규정) — 캡처 교시로 위치별 해결.
...
**= 5(LINE) + 5(T3) + 5(CROSS4) + 1(CORNER2) + 1(CORNER3) = 17종** (코너 거울 L/R 분리 시 최대 19).
...
| `T3-FLOOR` | T3 (3갈래) | `B` | 수식/캡처 | **신규** |
| `T3-CEIL` | T3 (3갈래) | `T` | 수식/캡처 | **신규** |
| `T3-WALL` | T3 (3갈래) | `SM` `PM` `F` `A` | 수식/캡처 | **신규** |
| `T3-CHMR-LO` | T3 (3갈래) | `SL` `PL` | 수식/캡처 | **신규** |
| `T3-CHMR-UP` | T3 (3갈래) | `SU` `PU` | 수식/캡처 | **신규** |
| `CROSS4-FLOOR` | CROSS4 (4갈래) | `B` | 수식/캡처 | |
| `CROSS4-CEIL` | CROSS4 (4갈래) | `T` | 수식/캡처 | |
| `CROSS4-WALL` | CROSS4 (4갈래) | `SM` `PM` `F` `A` | 수식/캡처 | |
| `CROSS4-CHMR-LO` | CROSS4 (4갈래) | `SL` `PL` | 수식/캡처 | |
| `CROSS4-CHMR-UP` | CROSS4 (4갈래) | `SU` `PU` | 수식/캡처 | |
...
> **교시/실행 열**: *도면 솎기* = 면 DXF 단면(X-Z) 솎기(LINE 현행). *수식/캡처* = 평탄면이면 패턴 수식 생성,
> 굴곡면이면 X-Y 6-DOF 캡처 교시(`/inspection-points`) — **이원화**. *캡처 교시 전용* = 브릿지 플레이트 실측을
> 6-DOF 캡처로만 담는다(수식 불가). CORNER3 온보드 현행은 고정 슬롯(`corner3.*`, §9) — 추후 재정의 대상.
...
3. **~~4점 십자를 직선과 별도 타입으로 둘지~~** — **확정(2026-09-15): 독립 5종**(LINE·T3·CROSS4·CORNER2·CORNER3). T자(3갈래)를 신규 추가, 코너를 2면/3면으로 분리.
...
5. **profileId enum 구현** — `ref.area_task.profile_id` / 액션 `param_schema` 반영.
   > HD_AMR 측 진행(2026-09-15): 정본 카탈로그(§5) 중 **현행 11종**(`LINE-*` 5 + `CROSS4-*` 5 + `CORNER3`)이
   > 온보드 DB(`InspectionRecipes`, id = §5 문자열 그대로)로 구현되어 `(seamType, wall_code)` → 레시피 매핑·실행
   > 배선 완료(VDA5050_INTERFACE_SPEC §8.5.1 (5)). **`T3-*`·`CORNER2` 는 미시드(신규 — §9-6 후속).**
...
6. **신규 타입(T3·CORNER2) 구현** `[진행 중]` — 카탈로그 정본화(2026-09-15, 독립 5종) 완료. 후속:
   - **T3(3갈래)**: CROSS4와 동형 **이원화** — 평탄면은 3-arm 패턴 수식 생성(`CrossPatternGenerator` 3-arm 확장), 굴곡면은 X-Y 6-DOF 캡처 교시(`/inspection-points`). `T3-*` 5종 레시피 시드.
...
7. **`seamType` enum 계약 확장** `[협의 N13]` — 현행 계약 enum 은 `LINE`/`CROSS`/`CORNER` 3값. 본 5종을 계약에 싣는다면
   `T`(3갈래) 추가 + `CORNER` → `CORNER2`/`CORNER3` 분리가 필요하다(ACS 발행값 변경 = 계약 변경). 반영 방식·시점은 N13 협의. 그 전까지 온보드 카탈로그(§5)만 5종으로 정본화하고, ACS 계약 enum 은 무변경.
