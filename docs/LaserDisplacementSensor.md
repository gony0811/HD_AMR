...
- **무엇을 측정하나**: 대상 표면 3점에 대해 거리(mm)를 측정하여 평면의 기울기/높이 보정에 활용. 센서 원시 출력은
  기준거리(300mm) 중심 변위(±150)이나, 아핀 캘리브레이션(`MeasurementOffsetMm` + `MeasurementScale`)으로
  **출사부 기준 절대거리**(예: 150~450mm)로 환산해 표시한다.
...
| 항목 | 바이트 오프셋 | 형식 |
|---|---|---|
| 측정값(원시, 기준거리 중심 변위) | `48 + (N-1)*4` (CH1=48, CH2=52, CH3=56) | 32bit signed int → 물리값 = `MeasurementOffsetMm + raw×MeasurementScale` |
...
| `ReconnectDelayMs` | `5000` | 재접속 시도 간격 |
| `MeasurementScale` | `-0.00001` | 원시값 스케일. 물리값 = `Offset + raw×Scale`. **음수** = 캘리브레이션 방향(raw+ = 가까워짐 → 거리 감소) |
| `MeasurementOffsetMm` | `300.0` | 아핀 오프셋(mm) = 센서 기준거리. 출사부 기준 절대거리 표시. `0`이면 변위 그대로 |
| `TiltStandoffMm` | `0.0` | 평면 pose 중심 Z 오프셋. 값이 절대거리면 0 → 중심 Z가 실제 거리 |

> **캘리브레이션 예**: 출사부 300mm=0/바깥쪽 −150/안쪽 +150 변위를 절대거리로 표시하려면 `MeasurementOffsetMm=300`,
> `MeasurementScale` 부호를 음수로(안쪽=가까움=거리 작아짐). 결과: 0→300, −150→450, +150→150mm.
> 단, **영점(Zero set)** 을 걸면 장치 변위가 0이 되어 절대거리 표시로는 오프셋값(300)이 나타난다.
> 나머지 필요한 설정(CIP 오브젝트 ID, RPI 등)은 확정 후 이 POCO 와 섹션에 추가.
...
- `LaserDisplacementSensorSettings Settings` — 접속 설정.
- **TODO** `Task<double> ReadMeasurementAsync(CancellationToken)` — 클라이언트에 스텁만 존재
  (`NotImplementedException`). 3장 CIP 매핑 확정 후 구현.

**동작 규약**: 다른 장치(AMR/Cobot/Camera)와 동일하게 호스트 기동과 동시에 **상시 자동 접속**하며,
연결이 끊기면 `ReconnectDelayMs` 간격으로 자동 재접속한다(수동 연결/해제 없음).

## 6. UI

- 라우트: `/laser` (내비게이션 "Laser Sensor" 버튼 → `HD_AMR.Web/Components/Pages/LaserDisplacementSensor.razor`).
- 통신 연결 카드: 상태 배지(연결됨/끊김)와 대상 IP:포트(참고용 정적 표기), 오류 표시. 상시 자동 접속이라 수동 연결/해제·IP 편집 UI 없음(대상은 appsettings 로 설정).
