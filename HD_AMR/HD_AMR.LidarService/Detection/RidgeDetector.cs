...
internal sealed class RidgeDetectorOptions
{
    ...
    /// <summary>
    /// 코러게이션 간격(mm). 실물 370mm. 0 이면 검사하지 않는다.
    ///
    /// <b>후보를 두 번 세지 않기 위한 물리 제약이다.</b> 봉우리 폭이 창보다 넓으면 창을 떼어낸
    /// 뒤 어깨가 남고, 그 어깨가 바로 옆 창으로 채택된다. 실측에서 무리 간격 11개 중 10개가
    /// 창 폭(100mm)과 정확히 일치했다 — 피치 370mm 인 실물에서 나올 수 없는 값이고, 그렇게
    /// 만들어진 가짜 후보가 선택되면 좌표가 400~500mm 튄다.
    ///
    /// 두 코러게이션은 반드시 이만큼 떨어져 있으므로, 절반보다 가까운 두 봉우리는 같은 것의
    /// 다른 부분이다.
    /// </summary>
    public double CorrugationPitchMm { get; set; } = 370.0;
    ...
    /// <summary>
    /// 두 번째 이후 봉우리를 인정할 최소 비율(가장 큰 봉우리 대비).
    ///
    /// 코러게이션을 가장 밀도 높은 것부터 하나씩 떼어내는데, 실물이 다 떨어지고 나면 남은
    /// 산발 점에서도 창 하나에 얼마간은 담긴다. 그 잔여물을 코러게이션으로 착각하지 않도록
    /// 봉우리 높이가 첫 봉우리의 이 비율 밑으로 떨어지면 멈춘다.
    ///
    /// 화각 가장자리에 걸린 코러게이션은 점이 적으므로 너무 높게 잡으면 놓친다.
    /// </summary>
    public double PeakRelativeThreshold { get; set; } = 0.25;
